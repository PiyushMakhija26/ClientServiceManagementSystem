...
export default function AllocateWorkPage() {
  const [selectedRequestId, setSelectedRequestId] = useState<number | null>(REQUESTS_TO_ALLOCATE[0].id);
  ...
  const handleAllocateWork = () => {
    if (selectedRequest && selectedMemberId) {
      setAllocated(true);
      setTimeout(() => {
        setSelectedMemberId('');
        setAllocated(false);
